...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      const { data } = await axios.get("https://elections-backend-j8m8.onrender.com/api/admin/voters");
      const users = data.voters || data || [];
      setUsers(users);
  ...
  const fetchCandidates = async () => {
    try {
      const { data } = await axios.get("https://elections-backend-j8m8.onrender.com/api/candidates");
  ...
  const handleCandidateImageUpload = async (candidateId, imageUrl) => {
    setCandidates((cs) =>
      cs.map((c) =>
        (c.id === candidateId || c._id === candidateId) ? { ...c, image: imageUrl } : c
      )
    );
  };
  ...
  const handleDeleteCandidate = async (id) => {
    ...
    try {
      await axios.delete(
        `https://elections-backend-j8m8.onrender.com/api/candidates/${id}`
      );
